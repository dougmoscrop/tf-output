...
module.exports = function getTerraformOptions(args) {
  ...
  const argv = yargs(terraformArgv)
    ...
    .option('allow-unapplied-plan', {
      alias: 'u',
      default: false,
      describe: 'skip the terraform plan, which checks for additions and changes',
      type: 'boolean'
    })
    .usage('$0 <list of dirs> [options] [-- command --flag]')
    .example('$0 module_1 module_2 -f json', 'print the module_1 and module_2 outputs as json to stdout')
    .example('$0 module_1 module_2 -- sls deploy', `exec 'sls deploy' command with environment variables available`)
    .argv;
